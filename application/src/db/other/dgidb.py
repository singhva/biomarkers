...
class DGIAPI:
    ...
    def create_and_post_request(self):
        self.create_request()
        self.post_request()
    ...
    @classmethod
    def get_agents(self, gene_syms):
        args = parse_args()
        args.genes = gene_syms
        dgidb = DGIAPI(args)
        matches = dgidb.create_and_post_request()
        drugs = []
        for match in matches:
            for interaction in match['interactions']:
                drugs.append(interaction['drugName'])
        return drugs
